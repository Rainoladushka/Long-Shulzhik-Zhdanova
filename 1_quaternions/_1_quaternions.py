...
class Quaternion:
    ...
    # Addition
    def __add_Quaternion__(self, other):             
    ...
    # Multiplication
    def __multiply_Quaternion__(self, other):            
    ...
    # norm
    def __norm_Quaternion__(self):
        return math.sqrt(self.a**2 + self.b**2 + self.c**2 + self.d**2)

    # Inverse
    def __inverse_Quaternion__(self):
        norm_sq = self.__norm_Quaternion__()**2
        if norm_sq == 0:
    ...
    def rotate_vector(self, vector):
        ...
        rotated = self.__multiply_Quaternion__(vec_quat).__multiply_Quaternion__(self.__inverse_Quaternion__())  # rotation process
...
def __add_quaternions__():
    try:
        ...
        q_sum = q1.__add_Quaternion__(q2)
...
def __multiply_quaternions__():
    try:
        ...
        q1 = Quaternion(a1, b1, c1, d1)
        q2 = Quaternion(a2, b2, c2, d2)
        q_multiply = q1.__multiply_Quaternion__(q2)
...
def __inverse_quaternions__():
    try:
        ...
        q1 = Quaternion(a1, b1, c1, d1)
        q_inversion = q1.__inverse_Quaternion__()
...
# btn_add
btn_add = tk.Button(root, text="Add quaternion", command=__add_quaternions__,width=16,height=2)
btn_add.grid(row=4, column=0)

#btn multiply 
btn_multiply = tk.Button(root, text="multiply quaternion", command=__multiply_quaternions__,width=16,height=2)
...
#btn norm 
btn_norm = tk.Button(root, text="norm quaternion 1", command=__norm_quaternions__,width=16,height=2)
btn_norm.grid(row=5, column=0)

#btn inverse
btn_inverse=tk.Button(root, text="inverse quaternion 1", command=__inverse_quaternions__,width=16,height=2)
